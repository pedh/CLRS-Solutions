...
** 7-1
   1. omitted
   2. A[p..i] and A[j..q] is the elements already accessed, A[i..j] is the
      elements never accessed.
   3. \(p - 1 \leq i < j < r\)
...
** 7-4
   1. TAIL-RECURSIVE-QUICKSORT use a recursion to handle the left part, and a
      iteration to handle the right part.
...
** 7-5
   ...
   4. The median-of-3 method only improve the possibility of choosing a good
...
** 7-6
   ...
   2. Assume partition on n elements return k overlapped elements
      ...
      k is not strongly related to n, so the expected running time is
      \(\Theta(n\lg n)\).
